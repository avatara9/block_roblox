...
if (-not (Test-IsAdministrator)) {
    try {
        ...
        Add-Content -Path $temporaryErrorLog -Encoding UTF8 -Value (
            '{0:yyyy-MM-dd HH:mm:ss} Требуются права администратора.' -f
            (Get-Date)
        )
    }
    catch {
...
function Add-RobloxFilesFromDirectory {
    ...
    try {
        ...
        try {
            ...
            Write-Log "Не удалось просканировать каталог: $Directory" 'WARN'
        }
    }
}
...
function Get-RobloxStorePackages {
    try {
        ...
        Write-Log 'Не удалось прочитать Microsoft Store-пакеты Roblox.' 'WARN'
        return @()
    }
}
...
function Stop-CurrentlyRunningRoblox {
    ...
    if ($stopped -gt 0) {
        Write-Log "Остановлено процессов Roblox: $stopped"
    }
}
...
function Add-RobloxFirewallRules {
    ...
    foreach ($programPath in $ExecutablePaths) {
        ...
        try {
            New-NetFirewallRule `
                ...
                -Description (
                    'Блокировка исходящего трафика Roblox: ' +
                    $programPath
                ) `
                -Group $FirewallGroup `
                -Direction Outbound `
            ...
            Write-Log "Не создано правило Firewall: $programPath" 'WARN'
        }
    }
    ...
    if ($packages.Count -gt 0) {
        try {
            ...
            foreach ($package in $packages) {
                try {
                    ...
                    New-NetFirewallRule `
                        ...
                        -Description (
                            'Блокировка Store-пакета Roblox: ' +
                    ...
                    Write-Log (
                        'Не создано правило Store-пакета: ' +
                        $package.PackageFamilyName
                    ) 'WARN'
                }
            }
            ...
            Write-Log 'Не подготовлена блокировка Store-пакетов.' 'WARN'
        }
    }
...
function Set-FirefoxBlockPolicy {
    ...
    if (Test-Path $policyFile -PathType Leaf) {
        try {
            ...
            Write-Log (
                "Некорректный JSON Firefox оставлен без изменения: $policyFile"
            ) 'WARN'
...
function Set-RobloxSoftwareRestrictionPolicies {
    ...
    foreach ($rulePath in (Get-RobloxSrpPaths)) {
        ...
        $expectedDescription = "$SrpDescriptionPrefix запрет $rulePath"
...
function Invoke-ComputerPolicyRefresh {
    try {
        ...
        Write-Log 'Не удалось выполнить gpupdate.' 'WARN'
    }
}
...
function Stop-RobloxPid {
    ...
    try {
        ...
        Write-GuardLog "Завершён $description"
    }
    catch {
...
"@
    ...
    Write-GuardLog 'Событийный сторож запущен.'
...
function Install-ProcessGuardTask {
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Description (
            'Событийно завершает процессы Roblox без постоянного опроса.'
        ) `
        -Force | Out-Null
...
function Install-AutoRefreshTask {
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Description (
            'Раз в 6 часов ищет новые версии Roblox и обновляет Firewall.'
        ) `
        -Force | Out-Null
}
...
function Remove-RobloxTasks {
    try {
        ...
        Write-Log 'Не удалось полностью удалить фоновые задачи.' 'WARN'
    }
}
...
function Install-BrowserPolicies {
    ...
    foreach ($firefoxDirectory in (Get-FirefoxInstallDirectories)) {
        try {
            ...
            Write-Log (
                "Не настроена политика Firefox: $firefoxDirectory"
            ) 'WARN'
        }
    }
}
...
function Remove-BrowserPolicies {
    ...
    foreach ($firefoxDirectory in (Get-FirefoxInstallDirectories)) {
        try {
            ...
            Write-Log (
                "Не очищена политика Firefox: $firefoxDirectory"
            ) 'WARN'
        }
    }
}
...
function Remove-RobloxBlock {
    Write-Log "Начато удаление версии $ScriptVersion."
    ...
    try {
        ...
        Write-Log 'Не удалось полностью удалить правила Firewall.' 'WARN'
    }
    ...
    Write-Log 'Блокировка Roblox удалена.'
}
...
if (-not $RefreshOnly) {
    if ([string]::IsNullOrWhiteSpace($PSCommandPath)) {
        Write-Log 'Не определён путь текущего скрипта.' 'ERROR'
        exit 2
    }
...
try {
    ...
    Write-Log 'Не удалось загрузить NetSecurity или обновить Firewall.' 'ERROR'
    exit 3
}
...
if (-not $RefreshOnly) {
    ...
    try {
        ...
        Write-Log 'Не удалось полностью установить SRP.' 'ERROR'
    }
    ...
    try {
        ...
        Write-Log 'Не удалось создать задачу обновления.' 'ERROR'
    }
    ...
    try {
        ...
        Write-Log 'Не удалось создать событийный сторож.' 'ERROR'
    }
}
...
try {
    ...
    if ($disabledProfiles.Count -gt 0) {
        Write-Log (
            'Windows Firewall отключён для профилей: ' +
...
Write-Log (
    "Версия=$ScriptVersion; RefreshOnly=$RefreshOnly; " +
    "найдено EXE=$($executables.Count); " +
    "добавлено правил Firewall=$newFirewallRules."
)
